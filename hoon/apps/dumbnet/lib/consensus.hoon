...
++  validate-page-without-txs
  ...
  ::  An %ai-pow block's target must lie inside the minable domain, whichever
  ::  path produced it. +compute-target-ai-asert caps its own output at
  ::  +max-ai-target-atom, but a block below the ASERT phase inherits the epoch
  ::  target, which is not capped and on a fresh chain is the genesis target.
  ::
  ::  Above the cap the verifier's shape-scaled jackpot threshold leaves the 256
  ::  bits it is computed in and fail-closes, so such a block is unminable: it is
  ::  rejected however much work went into it. Saying so here names the reason
  ::  instead of surfacing it as an opaque pow failure.
  ?:  ?&  ?=([%ai-pow *] (need ~(pow get:page:t pag)))
          %+  gth  (merge:bignum:t ~(target get:page:t pag))
          max-ai-target-atom:t
      ==
